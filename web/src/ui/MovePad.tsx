        ...
        faceOrder.map((face) => {
          ...
          return (
            <button
              ...
              className="relative flex h-10 items-center justify-center gap-1.5 rounded-lg border border-line font-mono text-[13px] text-ink transition-[background-color,border-color,transform] duration-100 hover:border-line-strong hover:bg-ink/[0.04] active:scale-[0.94] active:bg-ink/[0.08]"
            >
              {suffix === '' && (
                <span className="size-1.5 rounded-full" style={{ backgroundColor: faceColors[face] }} aria-hidden />
              )}
              {move}
            </button>
          )
